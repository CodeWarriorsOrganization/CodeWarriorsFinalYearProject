...
public class RandomMarketComponent {
	 
    
    
	public static int[][] generateRandomMarketStockRandomValue() {
	    ...
	    int[][] randomMarke = new int[numberofturns][numofcompanies];
	   //--variables declaration end
		
		//------------------------------Random MARKET COMPONENT ALGORITHM BEGIN--------
	    //      outer for loop handles number of turns
	        for (int z = 0; z < randomMarke.length; z++) {

	    //          first inner for loop handles number of companies/stocks
	            for (int j = 0; j < randomMarke[z].length; j++) {
	    ...
	    //              assigned an array with dummy values because should maintain starting array without having '0's ;) #trick
	                int[] arrayRandomCompanies = {55, 55, 55, 55, 55, 55};
	    ...
	    */
	    ...
	    //              set randomly generated value to parent array
	                randomMarke[z][j] = arrayfinalizedturn[j];
	            }
